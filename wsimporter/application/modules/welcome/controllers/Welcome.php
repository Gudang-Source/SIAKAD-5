...
class Welcome extends CI_Controller {
	...
	{
		// $this->getProdi();
		$data_kur = $this->App_model->get_query("SELECT * FROM tb_kurikulum ORDER BY status ASC")->result();
		$data['data_krs'] = $data_kur;
		$data['site_title'] = 'Home';
		$data['title_page'] = 'Selamat Datang';
		$data['assign_js'] = 'js/welcome_dt.js';
		$data['assign_modal'] = '';
		tampil('welcome_view',$data);
	}
	...
	public function loginas()
	{
		...
		$data['site_title'] = 'Login sebagai?';
		$data['title_page'] = 'Login sebagai?';
		$data['ket_page'] = '';
		$data['assign_js'] = '';
		$data['assign_modal'] = '';
		tampil('loginas_view',$data);
	}
		...
		if (($temp_error_code==0) && ($temp_error_desc=='')) {
			...
			foreach ($temp_table['result'] as $key) {
				$temp_data[] = array('no' => ++$i,
									...
									'aksi' => '
											<a href="'.base_url().'index.php/welcome/listdir/'.$key['table'].'" target="_blank" data-height="300" data-width="560" data-target="#modalku">

...
class Nilai_trans extends CI_Controller
{
    ...
    public function get_mhs_trans()
    {
      ...
      if ($page=='') {
        $data_krs = $this->App_model->get_query("SELECT * FROM v_mhs_transfer WHERE kd_prodi_now='".$this->session->userdata('level_prodi')."'")->result();
      }
      else {
        $data_krs = $this->App_model->get_query("SELECT * FROM v_mhs_transfer WHERE kd_prodi_now='".$this->session->userdata('level_prodi')."' AND nim LIKE '%".$cari."%' ")->result();
      }
